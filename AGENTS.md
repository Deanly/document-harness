...
- Treat the authoritative current DDD model as the shared business contract for customers, planners, architects, developers, and QA. Before delivery work, identify affected bounded contexts, load the role packet from `docs/_indexes/context-packets.yaml`, and verify exact model bytes through `docs/lib/domain-design-authority.mjs`. Route semantic changes and implementation-alignment review through the provider-neutral `ai-domain-expert` role. It is the highest supervisory authority below the human decision owner: it may inspect and challenge every delivery role, require code/model rework, and block checkpoint success, closeout, or release on unresolved meaning. It may promote routine, reversible, evidence-backed model changes only within an exact delegated-authority fence; it cannot create or widen that fence, make material/strategic decisions, or accept residual risk. Escalate those decisions to the Board with the problem, model expectation, implementation reality, business/engineering impact, options, AI recommendation, confidence, and exact human decision required.
...
- Reuse fresh in-scope goals, directives, decisions, and approvals instead of asking again each turn. Exercise bounded discretion for routine, reversible, low-risk implementation details that do not change the goal, acceptance, policy, domain meaning, or non-waivable boundary. Request new approval only for a material delta or human-only authority/risk boundary; explain the goal, work so far, why the decision is needed, what changes, options and impacts, and approval/non-approval effects in the user's language before showing technical fence metadata. Never require the user to copy an opaque token or exact phrase as the substance of approval, and block only the minimum dependent action while evidence-backed independent work may continue.
...
- The Board Domain surface is the human communication channel for AI Domain Expert supervision. It must project canonical model truth from `docs/design/` and exact delivery-linked supervision/decision receipts without authoring either. A model/code conflict remains a closeout blocker until implementation or model change is re-reviewed as `aligned`, or the human explicitly accepts an expiring temporary deviation.
