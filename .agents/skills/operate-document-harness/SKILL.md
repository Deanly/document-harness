...
4. Run the DDD preflight in `docs/guide/ddd-domain-design.md`: identify affected bounded contexts, load the actor-specific packet, and require authoritative/current exact model bytes to pass `docs/lib/domain-design-authority.mjs`. Route semantic changes and implementation-alignment review through `ai-domain-expert`. Customer, planner, architect, developer, and QA use the same model IDs through different role views; project/task/QA documents must declare their domain impact, traceability, and current supervision state.
5. Treat `ai-domain-expert` as the highest supervisory authority below the human decision owner. It may inspect and challenge every delivery role, require implementation/model rework, and block success, closeout, or release on unresolved meaning. Let it resolve routine, reversible, evidence-backed changes only inside an exact delegated-authority fence; it cannot decide material/strategic meaning, accept residual risk, or create/widen its authority. For a conflict, have it prepare a Board package with exact model/implementation evidence, business/engineering impact, options, recommendation, confidence, and the precise human decision required. Code/model change choices require a new aligned review; only an exact human-accepted expiring temporary deviation may close without alignment.
...
Keep `<displayName> / <repository>` visible as the fixed top-left identity. Expect configured `tabLabels` in the canonical key order `overview`, `domain`, `policies`, `guidelines`, `initiatives`, `review`, `execution`, `evidence`. The Domain tab first shows open AI Domain Expert supervision: problem, model expectation, implementation reality, business/engineering impact, options, AI recommendation/confidence, and exact human decision. It then projects the chosen minimum-sufficient model, validation/freshness, and evidence. It does not dump every entity or technical ID onto the first screen and does not grant model authority. Treat domain, policy, guideline, and initiative as independent first-class surfaces. Initiative directly links policy WHY and selected guideline HOW, while projects own `related_initiative` and the View reverse-indexes their status/path without inventing progress.
