...
- `docs/guide/goal-locked-completion.md`: goal lock, 기능 단위 완료, `design-only` 예외 규칙
...
5. `project`와 `task`는 기본 `Completion Mode: functional`로 유지하고, 예외적인 설계 작업만 명시적으로 `design-only`로 발급합니다.
