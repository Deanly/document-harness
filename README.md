...
- `docs/guide/ddd-domain-design.md`: domain expert가 exact model bytes를 검토·승인하고 각 역할이 같은 모델을 소비하는 계약
...
4. `docs/design/domain-landscape.md`와 `docs/design/context-map.md`에서 bounded context를 정하고, 각 context에 domain model·ubiquitous language·executable examples를 함께 작성합니다. AI 초안은 domain expert가 exact bytes를 승인하기 전까지 `review_requested`입니다.
