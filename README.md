...
이 하네스에서 `design`은 전체를 놓치지 않게 만드는 control surface, `initiative`는 정책·지침을 portfolio outcome으로 잇는 strategy surface, `project`와 `task`는 delivery와 실행에 집중하게 만드는 focus surface입니다.
...
- `docs/design/initiative-governance-plane.md`: 정책·지침 → 추진안 → project/task 계층과 approval/legacy bridge 계약
...
4. 설계 기준이 필요하면 `design` 문서를 만들고 `control-plane`, `ubiquitous-language`를 같은 변경 셋에서 갱신합니다.
...
8. exact policy/normative rule version은 initiative summary만 믿지 않고 task/QA에 계속 연결합니다.
...
14. `./docs/bin/validate-execution-loop.sh --all`과 `./docs/bin/harness-adopt verify --target .`로 installed execution/release surface를 확인합니다.
