...
When Codex needs to issue a numbered `initiative`, `project`, `task`, or `qa` document, it uses a dedicated issuer worktree whose clean `main` exactly equals latest `origin/main`. It invokes `docs/bin/issue-doc-bridge.sh` with the delivery workstream's immutable code baseline, branch and kind. The document bridge keeps product code byte-identical to that baseline while projecting latest main document-control bytes and the new draft. Initiative still needs the exact human issuance ref; Project and Task retain their active/approved lineage gates. Codex must not infer `I0001` or `P0001` as defaults.

The bridge commit and its receipt-finalization commit are merged as the same exact SHAs into both `main` and the delivery branch; they are not independently cherry-picked. Number allocation is confirmed only after the finalization commit is visible from both remote refs. Never merge unreleased `main` product code merely to receive a document. Task/checkpoint provenance records the immutable code baseline, document issuance revision, bridge revision, receipt, workstream kind and delivery branch.
