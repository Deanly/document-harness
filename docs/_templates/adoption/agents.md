...
- Treat `docs/design/` as DDD-only domain truth. Before project/task/QA work, identify affected bounded contexts, load the actor-specific packet, and verify approved/current exact model bytes through `docs/lib/domain-design-authority.mjs`; AI may draft but cannot approve a domain model.
...
- Never self-approve policy, domain models, exceptions, migration conflicts, risk acceptance, or weakened checks.
