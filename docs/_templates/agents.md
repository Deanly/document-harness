...
- Treat the authoritative current DDD model as the shared business contract for customers, planners, architects, developers, and QA. Route semantic changes and implementation-alignment review through `ai-domain-expert` and verify exact model bytes through `docs/lib/domain-design-authority.mjs`. It is the highest supervisory authority below the human decision owner: it may inspect and challenge every delivery role, require code/model rework, and block success/closeout/release on unresolved meaning. It may promote routine, reversible changes only inside an exact human-defined delegation fence; it cannot decide material/strategic meaning or accept risk. Board escalation must show the conflict, impacts, alternatives, AI recommendation and exact human decision required.
